...
def get_xmind_testsuites(xmind_file):
    ...
    解析xmind文件，其中由于xmind8版本和xmindzen版本不一致，导致xmind8解析图标字段为markers，
    xmindzen解析图标字段为makers，故作兼容将解析后的dict中的markers统一替换为makers
    """
    xmind_file = get_absolute_path(xmind_file)
    # 增加xmindzen支持
    if is_xmind_zen(xmind_file):
        xmind_content_dict = xmind_to_dict(xmind_file)
        # print(f"xmind_content_dict_zen: {xmind_content_dict}")
    else:
        workbook = xmind.load(xmind_file)
        xmind_content_dict = workbook.getData()
        xmind_content_json = json.dumps(xmind_content_dict[0]).replace("markers", "makers")
        xmind_content_dict = [json.loads(xmind_content_json)]
        # print(f"xmind_content_dict_xmind8: {xmind_content_dict}")
