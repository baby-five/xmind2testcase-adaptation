...
# print(os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + os.sep + 'xmind2testcase')
# print(sys.path)
import logging
import re
import arrow
...
from xmind2testcase.zentao import xmind_to_excel_file
...
def delete_record(filename, record_id):
    ...
    excel_file = join(app.config['UPLOAD_FOLDER'], filename[:-5] + 'xlsx') #此处修改，增加删除excel文件

    for f in [xmind_file, testlink_file, zentao_file, excel_file]: #此处修改，增加删除excel文件
        if exists(f):
            os.remove(f)
...
def delete_records(keep=20):
    ...
    for row in rows:
        ...
        excel_file = join(app.config['UPLOAD_FOLDER'], name[:-5] + 'xlsx') #此处修改，增加删除excel文件
